...
#SONAR_TOKEN= # access token from SonarCloud projet creation page -Dsonar.login=XXXX : provided from environment through the CI
SONAR_PROJECT_KEY=sonarcloud_example_cpp-xcodebuild-macos-otherci # project key from SonarCloud projet creation page -Dsonar.projectKey=XXXX
SONAR_PROJECT_NAME=sonarcloud_example_cpp-xcodebuild-macos-otherci # project name from SonarCloud projet creation page -Dsonar.projectName=XXXX
...
build-wrapper-macosx-x86 --out-dir build_wrapper_output_directory xcodebuild -project sonar_scanner_example.xcodeproj -configuration Release
